...
#shokukai("准教授")
#gakuseki-num("a23036")
#let title = "卒論執筆の基本と卒論用スタイルファイルrgt.styの利用方法"

// 概要ページ
#abstract[
本書は、卒業論文の書き方について概説したものである。論文を作成する為には、文章を作文し、作成した文を適切に組版する事によって、文書とする必要がある。本書では、論文の為の作文における基礎的な要点と、組版に使用するTeX（XeLaTeX）の初歩的な内容、XeLaTeXで卒業論文を記述する為の、rgt.sty（Ryukoku Graduation Thesis）の取り扱いに付いて説明する。

作文の説明では、論文の特徴と、文章を構成する要素（即ち文と段落）における諸注意を与えた。又、推敲が如何に大切かという点について説明した。推敲は、文章を読み返して改良する作業である。推敲は、実は難易度の高い作業であるので、作文と推敲の要点を、文と段落に分けて本書で概説した。そして、数式を文の要素として如何に扱うべきかについて論じた。

数式を伴った文章を組版する上で、TeXは一般的なソフトの一つである。TeXは文書における文章と組版命令を読み込む事で、文書の組版を実行する。従って、TeXで文書をある程度思い通りに組み上げる為には、組版命令を習得しなければらない。本書では、TeXに入門する方法と、自習する為に前提となる知識について解説した。一方で、卒業論文を作成する上で、直接利用しない知識は付録に収録している。又、本書では、卒業論文で要請されている書式の組版命令を収録したrgt.styについて概説した。本書の解説と、本書自体を凡例として見る事で、卒論を執筆する準備は粗方整うはずである。

付録では、文字コードとTeXエンジンについて触れた。
]
...
の二つの作業が必要である。本文では、第二章で作文の基礎知識について解説する。又、第三章では、文書を作成する為の組版ソフトであるTeXと卒業論文用のrgtファイルについて説明する。
...
*推敲は、論文を書く上で必要不可欠である。*作文をしていると、論文の主張を見失ったり、後述の文や段落の規則を破りがちである。この為、どこかのタイミングで必ず読み返しながら、上記の点を一通り確認しなければならない。

== 文の構成
文を構成する上で、*主語と述語は整合しなければならない*。文を書き下す際は、主語が読み手である場合を除いて、主語を明示すべきである。又、文を締め括る際は、主語に対応した述語を配置しなければならない。

*文を構成する単語の意味を、書き手は把握していなければならない。*単語の誤用は百害あって一利無しであるので、辞書を引くか、自分が把握している言葉に置き換えるべきである。

直前の文章を鑑みて、*必要な場合は、文頭に適切な接続詞や節を配置するべきである。*但し、接続詞は必ずしも配置する必要はない。接続詞はややもすると濫用しがちになり、読み手の理解の妨げとなる為、接続詞の使用は最低限度に留めるべきである。

*文の長さは、出来る限り短くすべきである。*長い文は、読み手によって読み辛いだけでなく、書き手にとっても混乱を来し易くなる。この為、文が長くなってしまう場合は、以下の対策を検討する。
#list[
不要な文言を削除する
同一文内の重複した部分を集約する
分割する
...
== 段落の構成
段落とは意味内容が纏まった文章の連なりである。*段落の趣旨を明示する為に、段落の最初と最後の文では、段落の趣旨を可能な限り直接扱うべきである。*最初の文で趣旨を触れる理由は、段落を読む上での方向を読者に示す事で、内容の理解を助ける為である。一方で、最初と最後の段落で趣旨が変わってしまう段落は、意味内容の纏まりがなく、段落としての体を成していない。従って、段落の趣旨は先頭と末尾で明示するべきである。

一方で、段落の趣旨を段落内で説明しない段落については、上記の規則は適用されない。例えば、項目を列挙する事が趣旨の段落は、段落の趣旨を段落内で説明しない段落である。この場合、段落の趣旨を先頭で明記していれば、末尾で趣旨に立ち返る必要はない。
...
#remark[
セミナーの場合、板書の時間を削減する目的で論理式の利用が推奨される場面が多い。一方、文章の場合は読み易さが重視される為、自然言語での説明が好まれる場面が多い。
]

= 卒論の組版について
この章では、XeLaTeXを用いて卒論の文書を成す方法について述べる。TeXにはエンジンと呼ばれる種類があるが、XeLaTeXはその一つである。エンジンについては付録で触れる。

== TeXの概要
TeXは、文章と組版の為の命令を読み込み、文書を作成するソフトである。TeXに読み込ませる為の文章を書き込んだテキストファイルをソースファイルという。TeXにソースファイルを入力し、解釈させる（この作業をコンパイルという）ことで、始めて文書を得ることができる。

TeXがソースファイルの読み込みに成功すると、組版の結果をファイルで出力する。XeLaTeXの場合、出力結果はPDFファイルとなる。他のエンジンの場合は付録で扱う。XeLaTeXでは、PDFに直接出力する為、PDFファイルに対する命令郡も用意されている。

TeXに入力するソースファイルに、組版の為の命令を直接書き下す点は、Microsoft Wordとは大きく異なる。Microsoft WordやGoogle Documentは、最終的に出力される文書を直視しながら文書編集が出来る様になっている。上記の様なソフトにおいても、組み版命令群が記録されたファイルがあるが、基本的にユーザーは組み版命令を直接触れることはない。一方で、上記のソフトでは、実際の組み版命令とユーザーが直視しているインターフェースを繋ぐ必要がある為、TeXの様なソフトに比べて演算機の容量をより消費する必要がある。

TeXの利点の一つは、ソースファイルを直接扱える点にある。ソースファイル自体はテキストファイルである為、編集の為のコストが非常に小さい。その代わり、TeX自体には、グラフィカルなインタフェースや、バックアップ機能、添削機能といったものは無い点を注意する必要がある。
